...
const SENTINEL_KEY = `storage-rls-test/sentinel-${randomUUID()}.txt`;
...
describe("Storage RLS — certificates bucket (integration)", () => {
  ...
  it("authenticated client cannot list the certificates bucket", async () => {
    ...
    const files = data ?? [];
    expect(files.find((f) => f.name.startsWith("sentinel-"))).toBeUndefined();
  });
  ...
  it("service_role bypasses RLS and can list the certificates bucket", async () => {
    ...
    expect(data?.find((f) => f.name.startsWith("sentinel-"))).toBeDefined();
  });
});
