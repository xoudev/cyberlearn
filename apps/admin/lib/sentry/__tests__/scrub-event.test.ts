...
describe("scrubEvent - URL scrubbing", () => {
  it("redacts ?token= query parameter from request.url", () => {
    const event = makeEvent({
      request: {
        url: "https://admin.cyberlearn.fr/auth/confirm?token=abc123def456ghi789jkl012mno345p",
      },
    });
  ...
  it("redacts ?code= and ?email= query parameters", () => {
    const event = makeEvent({
      request: {
        url: "https://admin.cyberlearn.fr/auth/callback?code=shortcode&email=user@example.com",
      },
    });
    const result = scrubEvent(event);
    const url = new URL(result.request!.url!);
